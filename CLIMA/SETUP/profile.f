...
C   SOLVE FROM THE GROUND UP
     ...
     & IDRY,IMCO2)
       !print*,'CFLAG',CFLAG !This is where cflag is set for every run JDW 2021

       
C This makes the convectve zone the same as the cold trap in the atmosphere. JDW 2021       
       FLAGCONVEC(J1)=cflag
       BETA(J1) = BETA1
       BETA(J1-1) = BETA2
       JCOLD = J1
...
C
   4  CALL SATRAT(T(JCOLD),PSAT)
      ...
      FSATUR = (PSAT/P(J))
      FH2O(J) = FSATUR*RELHUM(P(J)) !This is where the water mixing profile is calculated JDW 2021
