      ...
      SUBROUTINE IR(T,PF,P,FNC,eFNC,CGAS,eCGAS)
...
C JDW Subroutine now contains logic needed for double cloud column. 
c
...
c     & xkappa(8,12,55,8,3)
     ...
     4 ,TAUO2O2(ND), TAUH2H2(ND),SELF_ABSIR(NF,ND),FORN_ABSIR(NF,ND)  ! Added self and foreign broadening continuum matrices 8/30/2012
      ...
      Dimension eCGAS(ND,NGS),eFNC(ND),eTAUTOTAL(NF),eTAUGH2O(ND) !Added by JDW for EddySed water mmr. 
...
C       np = 1.E+1
       ...
       print*,'MAXNZ-NLAYERS',MAXNZ-NLAYERS
...
!       TWGHT = weightco2_h2oIR(K1)*weightco2_h2oIR(K2)*weightch4(K2)*weightc2h6(k0) ! with ethane 3/20/2012
        ...
        DO 11 IL = 1,NLAYERS
         ...
         eTAUGH2O(IL)=KAPPALAYER(I,K1,1,IL)*eCGAS(IL,6) !JDW for the EddySed mmr.
     ...
     & + TAUO2O2(IL)+TAUH2H2(IL)
      ...
      DO IL=1, NLAYERS
           ...
           FNCR = FNC(IL) ! Needed for rayley
            ...
            eFwater=EddyFI(1,IL) !EddySed rayley
            eFNCR=eFNC(IL)    !For EddySed rarley JDW
           ...
           CALL RAYLEY(eSIGR,AL2,eFwater,eFNCR) !Call rayley to output a eSIGR at Al2 for the different water mmr JDW
          ...
          ASYEDDY(IL)=eddyg0(I,IL)
          OMG0EDDY(IL)=eddyw0(I,IL)
          TAUEDDY(IL)=eddyopd(I,IL)   !JDW
...
!                endif
      ...
      do z=1,NLAYERS
            ...
            !print*,'size',size(OMG0IR),size(OMG0EDDY)
            ...
            !Additional Sophistication: The Delta Approach test, from Toon et al. 1989. 
            !ASYIR_cloudy(z)=ASYIR_cloudy(z)/(1+ASYIR_cloudy(z)) !ASY_prime.
            !OMG0IR_cloudy(z)=

            !There is additional approximation here, or the need to fix the RT all together. 
...
C     & FUP,FDN,BPLANCK,TAUTOP, I ,K1, IL)
      ...
      CALL DELTA2STRIR(SRFALBIR,ASYIR_cloudy, !JDW OMG0IR_cloudy is what breaks this, for some reason. 
     & TAULAMIR_cloudy,OMG0IR_cloudy,
     & FUP_cloudy,FDN_cloudy,BPLANCK,
     & TAUTOP, I ,K1, IL)
      !print*,'fdn_cloudy',fdn_cloudy(10)
      CALL DELTA2STRIR(SRFALBIR,ASYIR_clear,
     & TAULAMIR_clear,OMG0IR_clear,
     & FUP_clear,FDN_clear,BPLANCK,
     & TAUTOP, I ,K1, IL)
...
!55555      CONTINUE
      ...
      DO IL=1, NLAYERS
          ...
          CALL RAYLEY(SIGR, AL2,Fwater,FNCR) ! Call rayley to output SIGR at a given altitude and wavelength, inputting AL2. 5/8/2011
           ...
           TAUSIR(IL) = TAUASIR(IL) + TAUR(IL)
          TAULAMIR(IL) = TAUAEXTIR(IL) + TAUGIR(IL) + TAUSIR(IL)
          !TAULAMVIS(IL) = TAUEXTIR_cloudy(IL) + TAUSIR(IL)
...
!         ,'
          ...
          OMG0IR(IL) = AMIN1(OMG0IR(IL),0.9999)
...
c jfk  6/25/08  Include TAUTOP in the call sequence to do the upper BC
      TAUTOP = TAULAMIR(1)*PF(1)/(PF(2)-PF(1))
      TAUTOP = AMIN1(TAUTOP,1.)
      !Need to also add in the rayleigh scattering from the EddySed mmr JDW


      do z=1,NLAYERS

      ASYIR_cloudy(z)=ASYEDDY(z)!ASYIR(z) + ASYEDDY(z) !This breaks the code, Like the IR treatment of the clouds is way too intense. 
      TAULAMIR_cloudy(z)=TAUEDDY(z)!TAULAMIR(z) + TAUEDDY(z)
      OMG0IR_cloudy(z)=OMG0EDDY(z)!OMG0IR(z) + OMG0EDDY(z)
      !print*,'ireddy-ir',omg0IR-omg0ir_cloudy
      OMG0IR_cloudy(z) = AMIN1(OMG0IR_cloudy(z),0.99999)
      !print*,OMG0IR
      OMG0IR_cloudy(z) = AMAX1(OMG0IR_cloudy(z),1.E-5)
      ...
      !if (isnan(OMG0IR_cloudy(z))) stop '"x" is a NaN'
      !print*,I,z,OMG0IR_cloudy(z)-OMG0IR_clear(z)

      enddo
      !print*,'TAULAMIR_cloudy',TAULAMIR_cloudy
      TAUTOP_cloudy=TAULAMIR_cloudy(1)*PF(1)/(PF(2)-PF(1))
      TAUTOP_cloudy=amin1(TAUTOP_cloudy,1.)
...
C     & FUP,FDN,BPLANCK,TAUTOP,I, K1,IL) !Commenting this out blows up temperature
      ...
      CALL DELTA2STRIR(SRFALBIR,ASYIR_clear, !JDW OMG0IR_cloudy is what breaks this, for some reason
     & TAULAMIR_clear,OMG0IR_clear,
     & FUP_cloudy,FDN_cloudy,BPLANCK,
     & TAUTOP, I ,K1, IL)
