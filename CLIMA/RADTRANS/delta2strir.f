...
C
     ...
     &     FUP(ND),FDN(ND),BPLANCK(ND)
      Double Precision E_heng(NZ),CGAM_heng(NZ)

C      dimension ASYIR_cloudy(NZ),TAULAMIR_cloudy(NZ),
C     & OMG0IR_cloudy(NZ),
C     & FUP_cloudy(ND),FDN_cloudy(ND)
     
C      dimension ASYIR_clear(NZ),TAULAMIR_clear(NZ),
C     & OMG0IR_clear(NZ),
C     & FUP_cear(ND),FDN_clear(ND)
C
...
C      GAM2(N) = SQ3*W0(N)*(1.-G(N))/2.
      ...
      GAM2(N) = W0(N)*(1.-G(N))
C
      !print*,'GAM',GAM1(N),GAM2(N)
      ALAM(N) = SQRT(GAM1(N)*GAM1(N) - GAM2(N)*GAM2(N))
      !print*,'ALAM',ALAM(N)
!      CGAM(N) = (GAM1(N) - ALAM(N))/GAM2(N)
      !print*,'ALAM',ALAM(N)
      if(isnan(ALAM(N))) stop '"ALAM" is nan'
      CGAM(N) = GAM2(N)/(GAM1(N)+ALAM(N))  ! Used the other definition for CGAM in Toon et al., (1989)
C=======================================================================================================
C     Here we update the CGAM(N) treatment according to Heng et al. (2018). 
C     This should help the Two-stream treatment in the presence of large aerosols
C     We use Table 1, Equation 24, and Equation 31 in Heng et al. (2018)

      E_heng(N) = 1.225 - 0.1582*G(N) - 0.1777*W0(N) - 0.07465*G(N)
     $ *G(N) + 0.2351*W0(N)*G(N) - 0.05582*W0(N)*W0(N)

      IF(E_heng(N) .lt. W0(N)) E_heng(N)=1.001 !Initial hack. Check OMG0IR JDW 2022 
      
      ! print*,'E_heng(N)',E_heng(N),N
      CGAM_heng(N) = (1-sqrt((E_heng(N)-W0(N))/(E_heng(N)-W0(N)*G(N))))/
     $(1+sqrt((E_heng(N)-W0(N))/(E_heng(N)-W0(N)*G(N))))
      ! print*,'CGAM(N)_heng',CGAM_heng(N),CGAM(N)
      ! print*,'CGAM(N) parts',(1-sqrt((E_heng(N)-W0(N))/(E_heng(N)-W0(N)*G(N))))
      ! print*,'CGAM(N) parrts2',(E_heng(N)-W0(N))/(E_heng(N)-W0(N)*G(N)),W0(N),G(N)
      ! print*,'CGAM(N) parrts2',W0(N),G(N)
