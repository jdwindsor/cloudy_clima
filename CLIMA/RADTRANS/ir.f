      ...
      SUBROUTINE IR(T,PF,P,FNC,FNC_cloudy,CGAS,CGAS_cloudy)
...
c-rr !3/23/11 put CIA matrix in IRDATA
     ...
     &,supsat,nsub_max,cld_hum1,clr_hum1,new_relhum(ND)
      !dimension FUPIR_cloudy(ND),FDNIR_cloudy(ND)
      !dimension FUPIR_clear(ND),FDNIR_clear(ND) 
...
****** AEROSOLS
      ...
      !   TAUAEXTIR_cloudy(IL)=QEXTIR(I,IL)*PI*r*r*DALT(IL)*np*1.E+5 ! this should be QEXTIR_cloudy
      !   TAUASIR_cloudy(IL)=OMG0AIR(I,IL)*TAUAEXTIR_cloudy(IL) !This should be omgoEddy

        !print*,'TAUASIR',TAUASIR_cloudy-TAUASIR
...
!                pause
    ...
    !    IF ((I.eq.15).and.(IL.ge.97))sumcont = sumcont + TAUCONTIN(IL)   ! only add approximately 1 km. ~ 0.1 or so for Earth
...
!       TWGHT = weightco2_h2oIR(K1)*weightco2_h2oIR(K2)*weightch4(K2)*weightc2h6(k0) ! with ethane 3/20/2012
        ...
        DO 11 IL = 1,NLAYERS
         ...
         TAUGH2O_cloudy(IL)=KAPPALAYER(I,K1,1,IL)*CGAS_cloudy(IL,6)!CGAS_cloudy(IL,6)
...
!        read(*,*)
      !  print*,CGAS(IL,4)*KAPPALAYEROZ(K4,IL),"Ozone CGAS"  
******* Ozone absorption
        IF (I.EQ.18) THEN
!                 sumoz = 0.  
      DO K4=1,8
...
c      TAUGIR(IL) =TAUGIR(IL) + TAUGOZ(IL)
     ...
     & + TAUO2O2(IL) + TAUH2H2(IL)
      ...
      !Make EddySed water mmr call here. 
      ...
      DO IL=1, NLAYERS
          ...
          ! print*,'call rayley'
           ...
           !print*,'TAUR',TAUR(IL)   !TAUR seems fine. JDW
           TAUSIR(IL) = TAUASIR(IL) + TAUR(IL)
           !print*,'TAUSIR',TAUSIR(IL) !TAUSIR seems fine. JDW

            TAUR_cloudy(IL)=EddySIGR*CRAY(IL)

            !print*,'TAUR',TAUR_cloudy-TAUR
            TAUSIR_cloudy(IL)=TAUASIR_cloudy(IL)+TAUR_cloudy(IL)
            !print*,'TAUSIR_cloudy',TAUSIR_cloudy(IL)!TAUSIR_cloudy seems fine. JDW

            ! Make EddySed water mmr Rayley here. JDW
...
C      CALL DELTA2STRIR(SRFALBIR,ASYIR,TAULAMIR,OMG0IR,
C     & FUP,FDN,BPLANCK,TAUTOP, I ,K1, IL)
      !print*,'fdn',fdn(10)
      !ASYIR_cloudy=0.001   !BAD!!! BIG OOF! YIKES! JDW
      !TAULAMIR_cloudy=1.0
...
C
     ...
     & +TWGHTT(K4)*FDN_clear(J)
      ...
      !& W(I)*FUPA_clear(J)
      !print*,'clear',FUPA_clear(J),FUPIR_clear(J),W(I)
      !print*,'cloudy',FUPA_cloudy(J),FUPIR_cloudy(J)
     
                  ENDDO !Something drastic changes here. 




            !print*,'ASYIR',ASYIR_clear(IL),ASYIR_cloudy(IL)!ASYIR_cloudy is non-dynamic. JDW
            !print*,'TAULAM',TAULAMIR_clear(IL),TAULAMIR_cloudy(IL) !TAULAMIR is off by about 10000 Issue seems to stem from the optical depth. JDW
            !print*,'OMG0IR',OMG0IR_clear(IL),OMG0IR_cloudy(IL),OMG0IR(IL) !OMG0IR is off by a lot too. JDW
...
!55555      CONTINUE
      ...
      !     print*,"TAUGIR 1",TAUGIR(IL)

          TAUGIR(IL) = TAUGCH4(IL)+TAUGH2O(IL)+ TAUGCO2(IL) + TPRIND(IL)  
     & +TAUCONTIN(IL)  + TAUGC2H6(IL)+ TAUH2N2(IL) !TAUGOZ(IL) increases the optical depth enough to fix the G0 problem. JDW 2022
     & + TAUO2O2(IL)+TAUH2H2(IL) !Somehow adding this fixes the current RT problems JDW 2022
      ! print*,"TAUGIR 2",TAUGIR(IL)
          !print*,'EddySigr',EddySIGR,SIGR

           !TAUR_cloudy(IL)=EddySIGR*CRAY(IL)
           !TAUSIR_cloudy(IL)=TAUASIR_cloudy(IL)+TAUR_cloudy(IL)
      !       if(doEddy)then
      !      if(TAUSIR(IL)-TAUSIR_cloudy(IL) .gt. 0.0001)then
      !      print*,'TAUSIR',TAUR(IL),TAUR_cloudy(IL),IL
      !      print*,'EddySIGR',SIGR,EddySIGR
      !      print*,'Eddywater',Fwater,EddyFwater
      !      print*,'watermmr ',FI(1,IL),FI_cloudy(1,IL)
      !      endif
      !       endif
           ! Make EddySed water mmr Rayley here. JDW
...
!      & +TAUSIR_cloudy(IL)
         ...
         !print*,'TAUGIR',TAUGIR_cloudy-TAUGIR
          !print*,'TAUR',TAUR
         !print*,'TAUSIR',TAUSIR-TAUSIR_cloudy !TAUSIR_cloudy is the big issue here. JDW
...
!           endif
          ...
          ASYIR(IL) = ASYAIR(I,IL) !ASY parameter, could it be interpreted here for _cloudy> JDW
          OMG0IR(IL) = TAUSIR(IL)/TAULAMIR(IL)
          ASYIR_cloudy(IL)=ASYAIR(I,IL)
...
c jfk  6/25/08  Include TAUTOP in the call sequence to do the upper BC
     ...
     & ASYIR_cloudy(z).gt.-0.99999)then 
      ...
      !print*,'ASYIR_cloudy',ASYIR_cloudy-ASYIR_clear
      !print*,'OMG0IR_cloudy',OMG0IR_cloudy

      !print*,'OMG0IR_cloudy',OMG0IR_cloudy-OMG0IR
      !print*,'shape ASYIR',shape(ASYIR) 
      TAUTOP_cloudy = TAULAMIR_cloudy(1)*PF(1)/(PF(2)-PF(1))
      TAUTOP_cloudy = AMIN1(TAUTOP_cloudy,1.)
      ...
      !ASYIR_clear=ASYIR
      !TAULAMIR_clear=TAULAMIR
      !OMG0IR_clear=OMG0IR
      !print*,ASYIR_clear-ASYIR_cloudy
C      CALL DELTA2STRIR(SRFALBIR,ASYIR,TAULAMIR,OMG0IR,
C     & FUP,FDN,BPLANCK,TAUTOP,I, K1,IL) !Commenting this out blows up temperature

      !print*,'ASYIR_last',ASYIR_cloudy(101),TAULAMIR_cloudy(101),
      !& OMG0IR_cloudy(101)
      !ASYIR_cloudy=0.001
      !TAULAMIR_cloudy=1.0
      
      ! The treatment above sets the OMG0IR_cloudy(z) vector to 0.999999 only! 
      ! do z=1,size(asyir)
      !       print*,"ASYIR 2",ASYIR_cloudy(z),OMG0IR_cloudy(z),z
      ! enddo
     ...
     & FUP_cloudy,FDN_cloudy,BPLANCK,TAUTOP_cloudy, I ,K1, IL)
      ...
      CALL DELTA2STRIR(SRFALBIR,ASYIR_clear,
     & TAULAMIR_clear,OMG0IR_clear,
     & FUP_clear,FDN_clear,BPLANCK,TAUTOP, I ,K1, IL)!There is clearly something wrong with the ClearSky optical properties. 
...
3111     FORMAT(1P5E14.5)
     ...
     & W(I)*FDNA_clear(J)
             FUPIR_clear(J)=FUPIR_clear(J)+
     & W(I)*FUPA_clear(J)
           ! PRINT*,'FDNA',FDNA_clear(J),FDNA_cloudy(J)
            !print*,FDNIR_cloudy(10),'cloudy' !This is very different than should be.
            !print*,FDNIR_clear(10)
            !print*,FDNIR(10),'fdnir'
            !print*,fdna(10),'fdna'
            !print*,fdna_cloudy(10),'cloudy' !This is very different.
...
!               endif
     ...
     & - (1.-fcloud)*FUPIR_clear(J) -fcloud*FUPIR_cloudy(J)) ! Outgoing IR at top of atmosphere   
             ENDIF
...
c 100    FORMAT(1X,1P10E12.5) !EWS - note label not used
 ...
 !     PRINT 100, TAUTOTAL
 !     pause
      !print*,'this is fup_clear',fup_clear(10)
      !print*,'this is fup_cloudy',fup_cloudy(10)
      !print*,'this is fup normal',fup(10)
      RETURN
      END
