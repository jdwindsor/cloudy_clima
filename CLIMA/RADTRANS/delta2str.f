...
C
     ...
     &     FUP(ND),FDN(ND)
      ...
      dimension ASY_cloudy(NZ),TAULAM_cloudy(NZ),OMG0_cloudy(NZ), !Do I need these?
     & FUP_cloudy(ND),FDN_cloudy(ND)
     
      dimension ASY_clear(NZ),TAULAM_clear(NZ),OMG0_clear(NZ),
     & FUP_cear(ND),FDN_clear(ND)
C
      ALB = SRFALB
      U0 = AMU0
C      U1 = 0.5  (Eddington value)
      ...
      U1 = 1./SQ3
      ! DO N=1,NZ
      ! U1 = 1.225 -0.1582*ASY(N) - !Need to make sure where the 1st eddington coefficient is used it is a vector JDW
      ! ENDDO
