...
C ************************************* ADDED VARIABLES/COMMON FOR CLOUD MODEL *************************************************************
...
C Here we copy the clearsky optical properties to the cloudy-sky optical properties JDW
C cloudy sky optical properties are labeled with "_cloudy"
C
C
C
...
c           TAUR(IL)= SIGR(I)*CRAY(IL)           
        ...
        TAUEXT_cloudy(IL) = TAUEXT(IL)



        TAUEXTID(IL) = 2*PI*r*r*DALT(IL)*np*1.E+5

        TAUEXTID_cloudy(IL) = TAUEXTID(IL)



        TAUAS(IL)=OMG0A(I,IL)*TAUEXT(IL)
        TAUAS_cloudy(IL) = TAUAS(IL)


        TAUS(IL)=TAUAS(IL)+TAUR(IL)
        TAUS_cloudy(IL) = TAUS(IL)
 1165  CONTINUE
C
       ...
       TAUASTOTAL_cloudy = TAUASTOTAL + TAUAS(IL)
...
C
       ...
       TAUEXTTOTAL_cloudy = TAUEXTTOTAL + TAUEXT(IL) !nothing more than copy-ing arrays here. JDW
       TAUASTOTAL_cloudy = TAUASTOTAL + TAUAS(IL)
...
!                   Bfsol(i) = 0.0d0
     ...
     &         + RADFLD*Bssol(i)) ! Self broadening coefficient with radiation field and temperature dependence included (cm^2/molecule)
                ...
                TAUCONTSOL_cloudy(IL) = ABSCONT*CGAS(IL,6)
!                TAUCONTSOL(IL) = 0.0d0
...
c  Add the NO2 absorption in here. Scale everything to CH4.
             ...
             TAUG_cloudy(IL) = SIGNO2(I)*CGAS(IL,2)*FNO2/FCH4 * 2.687E24
c         IF (I .eq. 7) THEN
...
c 99923 FORMAT("in solar.f:",I5, 1P10E10.2) !EWS - not used
            ...
            TAUG_cloudy(IL) = TAUG_cloudy(IL) + BETACH4(K3,I)*CGAS(IL,2)
            END IF           
...
C        Assumes Temp will never be lower than 112, log10(pressure) never lower than -4
     ...
     2    AFP*(1-AFT)*BETACH4NEW(II,2,5,K3)
      ...
      END IF          
            ...
            TAUG_cloudy(IL)= TAUG_cloudy(IL) + ANEWBETA*CGAS(IL,2)
...
c               TAULAM(IL) = AMIN1(TAULAM(IL),1000.)
               ...
               OMG0_cloudy(IL) = TAUS_cloudy(IL)/TAULAM_cloudy(IL)
C
