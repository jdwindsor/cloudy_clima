      ...
      SUBROUTINE IR(T,PF,P,CGAS,FI,AK)
C   AK is the gray atmosphere coefficient
c
...
c       DO 4 K3=1, 6 
c       DO 5 K0=1, 6
c-jdh   TWGHT = WEIGHT(K1,1)*WEIGHT(K2,2)*WEIGHT(K3,3)
c        TWGHT = WEIGHT(K1,1)*WEIGHT(K2,2)*WEIGHT(K3,3)*WEIGHTC2H6(K0)
c       TWGHT = WEIGHT(K1,1)*WEIGHT(K2,2)
        TWGHT = 1.
C  The above line is for a gray atmosphere
        DO 11 IL = 1,NLAYERS
...
c    & *(4.46*DM)/(16.*SM)
c         TAUGCH4(IL) = KAPPALAYER(I,K3,3,IL)*CGAS(IL,5)*2.687E24
...
*******
c
c   Replace this with a gray opacity
c         TAUGIR(IL) = TAUGCH4(IL)+TAUGCO2(IL)+TAUGH2O(IL)+TPRIND(IL)
c     & +TAUCONTIN(IL)+TAUGC2H6(IL)
        TAUGIR(IL) = AK*CGAS(IL,1)
c      IF (I.EQ.1) PRINT *,IL,TAUGIR(IL),CGAS(IL,1)
c
  11    CONTINUE
******* Ozone absorption
       IF (I.EQ.100) THEN
