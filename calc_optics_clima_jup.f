...
C   Read in Specti files from egp JDW!
      ...
      open(unit=001,file='cloud_optics/init_optics_bwni.txt')
      open(unit=002,file='cloud_optics/init_optics_wnoi.txt')
      open(unit=003,file='cloud_optics/init_optics_dwni.txt')
      open(unit=004,file='cloud_optics/init_optics_wave.txt')



C      do i=1,NSPC1IT
        read(001,*) bwni!bwni(i)
C        print*,'bwni',bwni(i)
C      enddo
      read(002,*) wnoi
      read(003,*) dwni
...
c        return
c      endif
      print*, 'init_optics(): do_optics, read_mie = ',
     & do_optics,read_mie
...
c   Read extinction and scattering coefficients for each condensing vapor
      ...
      if( read_mie )then
        ...
        do igas = 1, ngas
          ...
          filename = 'cloud_optics/' // this_gas(1:ns) // '.mieff'
...
c			  write(*,871)iwave,irad,igas,wlni(iwave),qext(iwave,irad,igas)
871			  format (3i5,2f15.5)
     ...
     $              iwave, wlni(iwave), wave_in*1d4, abs( 1 - wlni(iwave)/(wave_in*1d4) ),'shit'
                  stop 1
                endif
...
c   n_thetd is number of thetd values to consider
        ...
        do igas = 1, ngas
          ...
          this_gas = gas_name(igas)
          call dblank( this_gas, ns )
          filename = 'cloud_optics/' // this_gas(1:ns) // '.refrind'
